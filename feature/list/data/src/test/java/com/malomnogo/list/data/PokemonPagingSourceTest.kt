...
class PokemonPagingSourceTest {
    ...
    @Test
    fun `load returns null next key when list is empty`() = runTest {
    ...
    @Test
    fun `load filters out pokemon with id greater than first generation limit`() = runTest {
        fakeCloud.returnItems(
            listOf(
    ...
    @Test
    fun `load returns error when cloud source fails`() = runTest {
